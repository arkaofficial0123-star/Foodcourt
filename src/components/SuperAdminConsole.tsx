...
export default function SuperAdminConsole({
  ...
  const [editingUpiPermitted, setEditingUpiPermitted] = useState(false);
  ...
  const [allDeleteConfirmationInput, setAllDeleteConfirmationInput] =
    useState("");
  ...
  const handleVerifyMaster = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (inputUser === allowedId && inputPass === allowedPass) {
        sessionStorage.setItem("superadmin_global_auth", "true");
        setIsAuthenticated(true);
      } else {
      ...
      (snapshot) => {
        ...
        snapshot.forEach((docSnap) => {
          ...
          fetched.push({
            ...
            email: d.email || "",
            imageUrl: d.imageUrl || "",
            upiPermitted: d.upiPermitted === true,
            upiId: d.upiId || "",
          });
        });
        setRestaurants(
  ...
  const handleCreateRestaurant = async (e: React.FormEvent) => {
    ...
    if (
      restaurants.some((r) => r.id.toLowerCase() === slugClean.toLowerCase())
    ) {
    ...
    if (!/^(?:(?:\+|0{0,2})91)?[6789]\d{9}$/.test(cleanMobile)) {
      setCreateError(
        "Please enter a valid Indian mobile number (10 digits, e.g. 9876543210 or +91 9876543210).",
      );
      return;
    }
    ...
    try {
      ...
      setSuccessCreatedMsg(
        `Success! Branch "${payload.name}" (ID: ${slugClean}) has been created.`,
      );
      setActivePanel("list");
    ...
    try {
      ...
      const promises = restaurants.map(async (r) => {
        try {
          ...
          const deleteCats = catsSnap.docs.map((catDoc) =>
            deleteDoc(
              doc(db, "restaurants", targetId, "categories", catDoc.id),
            ),
          );
          await Promise.all(deleteCats);
  ...
  const handleSaveBranchSettings = async (oldId: string) => {
    ...
    if (/[^a-zA-Z0-9]/.test(slugClean)) {
      alert(
        "User ID can only contain alphanumeric characters (no spaces, no hyphens, no special characters).",
      );
      return;
    }
    ...
    try {
      if (oldId !== slugClean) {
        ...
        if (
          ...
          restaurants.some(
            (r) =>
              r.id.toLowerCase() === slugClean.toLowerCase() && r.id !== oldId,
          )
        ) {
          alert(
            `The User ID "${slugClean}" already exists. Please choose a different one.`,
          );
        ...
        const newData = {
          ...
          upiPermitted: editingUpiPermitted,
          upiId: editingUpiId.trim(),
        };
        ...
        const deleteOrdersPromises = ordersSnap.docs.map((orderDoc) => {
          return deleteDoc(
            doc(db, "restaurants", oldId, "orders", orderDoc.id),
          );
        });
  ...
  const handleSaveGlobalSettings = async (e: React.FormEvent) => {
    ...
    if (!targetId && !targetPass) {
      setGlobalSettingsError(
        "Please provide a new ID Identity or Secret Passkey to update.",
      );
      return;
    }
    ...
    try {
      ...
      await setDoc(doc(db, "settings", "security"), updateData, {
        merge: true,
      });

      setGlobalSettingsSuccess(
        "Super Admin security credentials updated successfully!",
      );
      setTimeout(() => {
        setGlobalSettingsSuccess("");
      }, 2000);
  ...
  if (!isAuthenticated) {
    return (
      <div
        className="flex min-h-screen flex-col items-center justify-center bg-[#050505] px-6 py-12 text-zinc-100"
        id="superadmin-auth"
      >
        <motion.div
        ...
        >
          <div className="text-center space-y-2">
            ...
            <h1 className="font-sans font-bold text-lg text-white tracking-wider uppercase">
              SUPER ADMIN LOGIN
            </h1>
          </div>
          ...
          <form onSubmit={handleVerifyMaster} className="space-y-4">
            <div className="space-y-1 text-left">
              <label className="font-mono text-[9px] uppercase tracking-wider text-zinc-555">
                username
              </label>
              <input
                type="text"
            ...
            <div className="space-y-1 text-left">
              ...
              />
              <p className="text-[10px] text-zinc-650 font-sans mt-1.5">
                Default profile:{" "}
                <code className="font-mono bg-zinc-950 px-1 py-0.5 rounded text-amber-500/80">
                  ADMIN
                </code>{" "}
                /{" "}
                <code className="font-mono bg-zinc-950 px-1 py-0.5 rounded text-amber-500/80">
            ...
            {authError && (
              <p className="text-xs text-rose-500 font-sans text-center">
                {authError}
              </p>
            )}
  ...
  return (
    <div
      className="min-h-screen bg-[#050505] pb-24 text-zinc-100 font-sans flex flex-col"
      id="superadmin-master-view"
    >
      ...
      <header className="sticky top-0 z-30 flex items-center justify-between border-b border-zinc-900 bg-[#0a0a0a]/90 px-8 py-5 backdrop-blur-md">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-sm font-bold tracking-tight uppercase text-amber-500">
                ADMIN
              </h1>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto w-full px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="p-4 rounded-xl border border-zinc-900 bg-neutral-900/5 space-y-1">
            ...
            <p className="text-2xl font-serif text-zinc-100 italic">
          ...
          <div className="p-4 rounded-xl border border-zinc-900 bg-neutral-900/5 space-y-1">
            ...
            <p className="text-2xl font-serif text-emerald-400 italic">
              All Servers Online
            </p>
          </div>
        </div>
              ...
              <div className="rounded-2xl border border-zinc-900 bg-zinc-950 p-6 space-y-6 shadow-2xl">
                <div className="space-y-1">
                  ...
                  <p className="text-zinc-500 text-xs font-sans">
                    Initialize a brand new business branch with comprehensive
                ...
                <form onSubmit={handleCreateRestaurant} className="space-y-5">
                  ...
                  {createError && (
                    <p className="text-xs text-rose-455 font-sans bg-rose-500/10 border border-rose-900/60 px-3 py-2 rounded-lg">
                      {createError}
                    </p>
                  )}
                ...
                <div className="rounded-2xl border border-rose-950/40 bg-rose-955/5 p-6 space-y-4 shadow-xl">
                  <div className="space-y-1">
                    ...
                    <p className="text-zinc-550 text-[10px] leading-normal font-sans">
                      Permanently wipe all registered dining branch databases in
                      a single action.
                    </p>
                  </div>
              ...
              <div className="bg-zinc-950/40 p-4 rounded-xl border border-zinc-900 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-4 text-xs font-mono">
                  ...
                  <div>
                    ...
                    <span className="text-zinc-300">{globalAdminPassword}</span>
                  </div>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNewGlobalAdminPassword(e.target.value)
                      }
              ...
              {isLoading ? (
                ...
                <div className="text-center py-20 border border-dashed border-zinc-900 rounded-2xl text-zinc-700 font-sans">
                  ...
                  <p className="text-sm font-medium">
                    No registered dining tenants detected.
                  </p>
                  <p className="text-xs text-zinc-600 mt-1">
                    Register a foodcourt brand to get started.
                  </p>
                </div>
              ) : (
                <div
                  className="grid grid-cols-1 gap-4"
                  id="tenant-instances-block"
                >
                    ...
                    return (
                      <div
                        ...
                        className={`rounded-2xl border transition-all duration-300 bg-neutral-900/5 ${
                          isExpanded
                            ? "border-amber-500/40 p-5 bg-neutral-900/20 shadow-lg"
                            : "border-zinc-900 p-4 hover:border-zinc-850"
                        }`}
                      >
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            <div
                              className={`transform transition-transform duration-200 text-zinc-500 group-hover:text-zinc-300 ${isExpanded ? "rotate-180" : ""}`}
                            >
                              <svg
                                className="w-4 h-4"
                                fill="none"
                            ...
                            {editingRepoId === restaurant.id ? (
                              ...
                              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 text-zinc-300 bg-zinc-950/40 border border-zinc-900/80 p-5 rounded-2xl shadow-xl font-sans">
                                ...
                                <div className="space-y-3.5">
                                  <h4 className="font-mono text-[9px] uppercase tracking-wider text-amber-500 font-bold">
                                    Edit Brand Identity
                                  </h4>
                                  <div className="space-y-1">
                                    <label className="font-mono text-[8px] uppercase tracking-wider text-zinc-500">
                                      Restaurant Name
                                    </label>
                                    <input
                                      type="text"
                                  ...
                                  <div className="space-y-1">
                                    <label className="font-mono text-[8px] uppercase tracking-wider text-zinc-505">
                                      Secret Passkey
                                    </label>
                                    <input
                                      type="text"
                                ...
                                <div className="space-y-3.5">
                                  ...
                                  <div className="space-y-1">
                                    ...
                                    <input
                                      ...
                                      value={editingMobile}
                                      onChange={(e) =>
                                        setEditingMobile(e.target.value)
                                      }
                                  ...
                                  <div className="space-y-1">
                                    ...
                                    <input
                                      ...
                                      className="bg-zinc-950 border border-zinc-805 text-xs text-zinc-100 px-3 py-2 rounded-lg w-full outline-none focus:border-amber-500/60 transition-colors"
                                      required
                                    />
                                  </div>
                                  <div className="space-y-1">
                                    ...
                                    <input
                                      ...
                                      className="bg-zinc-950 border border-zinc-800 text-xs text-zinc-100 px-3 py-2 rounded-lg w-full outline-none focus:border-amber-500/60 transition-colors"
                                      required
                                    />
                                  </div>
                                </div>
                                ...
                                <div className="flex flex-col justify-end">
                                  <div className="flex gap-2.5 pt-4 justify-end font-sans">
                                    ...
                                    <button
                                      type="button"
                                      onClick={() =>
                                        handleSaveBranchSettings(restaurant.id)
                                      }
                              ...
                              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 text-zinc-300 bg-zinc-950/20 border border-zinc-900/50 p-5 rounded-2xl shadow-lg font-sans">
                                ...
                                <div className="space-y-4 flex flex-col justify-between">
                                  <div>
                                    <span className="font-mono text-[8px] uppercase tracking-widest text-zinc-550 font-bold block mb-1">
                                      Restaurant Name
                                    </span>
                                    <h3 className="font-serif italic text-lg text-white font-extrabold tracking-wide drop-shadow-md">
                                      {restaurant.name}
                                    </h3>
                                  </div>
                                  <div className="space-y-2">
                                    <div className="flex items-center gap-1.5">
                                      <span className="text-[10px] uppercase font-mono text-zinc-550 font-bold">
                                        Branch Slug:
                                      </span>
                                      <span className="font-mono text-xs font-bold text-zinc-300 select-all tracking-wider px-1.5 py-0.5 rounded bg-zinc-955 border border-zinc-900">
                                        {restaurant.id}
                                      </span>
                                    </div>
                                ...
                                <div className="space-y-3 lg:border-x lg:border-zinc-900/50 lg:px-6">
                                  <h4 className="font-mono text-[9px] uppercase tracking-wider text-amber-500 font-bold flex items-center gap-1.5">
                                    Contact Details
                                  </h4>
                                  <div className="space-y-2.5 text-xs">
                                    <div className="flex items-start gap-2.5 text-zinc-400">
                                      ...
                                      <span
                                        className="leading-tight text-zinc-300"
                                        title={restaurant.address}
                                      >
                                        {restaurant.address ||
                                          "Main Street, Foodcourt Hub"}
                                      </span>
                                    </div>
                                    <div className="flex items-center gap-2.5 text-zinc-400">
                                      <Phone className="h-4 w-4 text-zinc-650 shrink-0" />
                                      <span className="font-mono text-zinc-300">
                                        {restaurant.mobile || "+1-234-567-890"}
                                      </span>
                                    </div>
                                    <div className="flex items-center gap-2.5 text-zinc-400">
                                ...
                                <div className="space-y-4 flex flex-col justify-between">
                                  <div className="space-y-3.5">
                                    <div>
                                      <span className="font-mono text-[8px] uppercase tracking-widest text-zinc-500 font-bold block">
                                        Terminal Passcode
                                      </span>
                                    ...
                                    {/* UPI Payment Credentials direct switch */}
                                    <div>
                                      <span className="font-mono text-[8px] uppercase tracking-widest text-zinc-500 font-bold block mb-1">
                                        UPI Payment Credentials
                                      </span>
                                      <div className="flex flex-col gap-1.5 bg-zinc-955 p-3 border border-zinc-900 rounded-lg max-w-xs">
                                        <div className="flex items-center justify-between gap-4">
                                          <span className="text-[10px] uppercase font-mono font-bold tracking-wider text-zinc-300 flex items-center gap-1.5">
                                            {restaurant.upiPermitted ? (
                                              <>
                                                <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse inline-block" />
                                                <span className="text-emerald-400">Permitted</span>
                                              </>
                                            ) : (
                                              <>
                                            ...
                                            onClick={async () => {
                                              try {
                                                await setDoc(
                                                  ...
                                                  { upiPermitted: !restaurant.upiPermitted },
                                                  { merge: true }
                                                );
                                              } catch (err: any) {
                                                alert("Failed to toggle UPI: " + err.message);
                                              }
                                            }}
                                            className={`relative inline-flex h-4.5 w-8 shrink-0 cursor-pointer rounded-full border border-transparent transition-colors duration-200 outline-none ${
                                              restaurant.upiPermitted ? "bg-amber-500" : "bg-zinc-800"
                                            }`}
                                          >
                                            <span
                                              className={`pointer-events-none inline-block h-3.5 w-3.5 transform rounded-full bg-zinc-950 shadow ring-0 transition duration-200 ${
                                                restaurant.upiPermitted ? "translate-x-3.5" : "translate-x-0"
                                              }`}
                                            />
                                          </button>
                                        </div>
                                        <div className="text-[9px] font-mono text-zinc-500 truncate" title={restaurant.upiId}>
                                          ID: <span className="text-amber-500 select-all font-bold font-mono">{restaurant.upiId || "None set"}</span>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                                  ...
                                  <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-zinc-900/60 font-sans">
                                    ...
                                    >
                                      <span
                                        className={`h-1.5 w-1.5 rounded-full ${restaurant.isEnabled ? "bg-emerald-400 animate-pulse" : "bg-rose-500"}`}
                                      />
                                      <span>
                                        Permission:{" "}
                                    ...
                                    <button
                                      onClick={() =>
                                        handleLaunchLocalBranch(restaurant.id)
                                      }
      ...
      <AnimatePresence>
        {restaurantToDelete && (
          ...
          >
            ...
            >
              <div className="space-y-2">
                <h3 className="font-serif italic text-xl text-rose-500">
                  Wipe Restaurant Trace?
                </h3>
                <p className="text-xs text-zinc-400 font-sans">
                  This will permanently delete the tenant branch{" "}
                  ...
                  ) and remove all administrative data.
                </p>
                <div className="bg-rose-950/10 border border-rose-900/30 rounded-xl p-3 text-[11px] text-rose-400 font-sans leading-relaxed">
                  ⚠️ <strong>Warning:</strong> This operation is irreversible
                  and instant. All current orders and database pointers will be
                  terminated.
                </div>
              </div>
              ...
              <div className="space-y-3">
                <label className="block font-mono text-[9px] uppercase tracking-wider text-zinc-500 font-bold font-mono">
                  Type <span className="text-zinc-350">CONFIRM</span> to
                  proceed:
                </label>
                <input
                  type="text"
              ...
              <div className="flex justify-end gap-3 font-sans">
                ...
                <button
                  ...
                  disabled={
                    isWiping ||
                    deleteConfirmationInput.trim().toUpperCase() !== "CONFIRM"
                  }
        ...
        {wipingAllModalOpen && (
          ...
          >
            ...
            >
              <div className="space-y-2">
                <h3 className="font-serif italic text-xl text-rose-500">
                  Wipe All Restaurant Branches?
                </h3>
                <p className="text-xs text-zinc-400 font-sans animate-pulse">
                  This will permanently delete{" "}
                  <strong className="text-zinc-100">
                    {restaurants.length} registered branches
                  </strong>{" "}
                  and wipe all of their administrative records.
                </p>
                <div className="bg-rose-955/20 border border-rose-900 px-3.5 py-3 rounded-xl text-[11px] text-rose-455 font-sans leading-relaxed">
                  🔥 <strong>CRITICAL WARNING:</strong> This is a master
                  destructive command. Clicking delete will wipe absolutely all
                  restaurant branch entries, configurations, menus, and orders
              ...
              <div className="space-y-3">
                <label className="block font-mono text-[9px] uppercase tracking-wider text-zinc-500 font-bold">
                  Type <span className="text-zinc-350">DELETE ALL</span> to
                ...
                <input
                  ...
                  onChange={(e) =>
                    setAllDeleteConfirmationInput(e.target.value)
                  }
                  className="w-full bg-zinc-900/40 border border-zinc-900 rounded-lg px-3 py-2 text-xs text-zinc-100 placeholder:text-zinc-700 font-mono uppercase focus:outline-none focus:border-rose-500"
                />
              </div>
              ...
              <div className="flex justify-end gap-3 font-sans">
                ...
                <button
                  ...
                  disabled={
                    isWipingAll ||
                    allDeleteConfirmationInput.trim().toUpperCase() !==
                      "DELETE ALL"
                  }
